...
import com.bta.api.service.data.SessionService;
import com.bta.api.service.data.UserService;
import com.bta.api.service.external.CredentialsService;
import com.bta.api.service.external.JwtTokenService;
...
public class AuthenticationController {
    ...
    public ResponseEntity<String> loginWithCredentials(@RequestBody LoginUserDto dto, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if (authentication.isAuthenticated()) {
                ...
                Users foundUser = userService.getUserByUsername(authentication.getName());
                SessionsDto sessionsDto = new SessionsDto();
                sessionsDto.setId(foundUser.getId());
                sessionsDto.setProvider(Provider.CREDENTIAL);
                sessionsDto.setJsonWebToken(jwtToken);
                sessionService.save(sessionsDto);
    ...
    public ResponseEntity<UsersDto> registerNewUser(@RequestBody RegisterUserDto dto) {
        try {
            if (!dto.getEmail().contains("@") || dto.getFirstName().equals("admin") || dto.getLastName().equals("admin")) {
